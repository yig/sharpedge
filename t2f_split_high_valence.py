"""
t2f_split_high_valence.py

Split polylines at high-valence vertices (valence ≥ 3 by default) in a 3D sketch OBJ file.

This helps improve downstream processing by breaking polylines that pass through complex junctions
into smaller, topologically simpler segments.

Functionality:
- Load a sketch from OBJ file (vertices and polylines)
- Analyze vertex valence from edges
- Identify high-valence vertices (valence ≥ threshold)
- Split polylines at these vertices
- Save result as new OBJ
- Optional: 3D visualization before/after, preview mode

Usage:
    python t2f_split_high_valence.py input.obj [output.obj] --plot --preview

Default output filename: input_split.obj
"""
