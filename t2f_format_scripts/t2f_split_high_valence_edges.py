"""
t2f_split_high_valence_edges.py

This script splits edges in the input sketch where both endpoints have valence > 2.
Such edges typically indicate ambiguous connectivity, and splitting them improves
topological consistency for further processing.

The script inserts midpoint vertices along these high-valence edges, updates the
polylines, and optionally previews the splitting result in a 3D plot.

Usage:
    python t2f_split_high_valence_edges.py input.obj output.obj
    python t2f_split_high_valence_edges.py input.obj output.obj --preview

Options:
    --preview   Show a preview of high-valence edges that would be split
                and save a visualization figure, without modifying the mesh.

Output:
    - A new OBJ file with midpoints inserted along selected edges.
    - A PNG visualization saved to `sketches_split_png/` (optional).
"""
import numpy as np
import argparse
